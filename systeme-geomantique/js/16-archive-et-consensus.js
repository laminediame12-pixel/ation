...
//
// « Les archives sont des matchs mixes matchs reels et Fifa. Dont on ne
// travaille plus sur ces archives compris. » Puis : « Supprime
// complètement ces archives, pas de trace. Je te donne de nouvelles
// matchs sur lesquelles on travaillera. »
//
// CE QUI A ÉTÉ TROUVÉ AVANT LA SUPPRESSION : le champ `esport` existait
// déjà sur 8 entrées de cette archive — mais tousCasBancV7() ne l'a
// JAMAIS filtré. Chaque mesure faite depuis le début du projet sur
// « l'archive » (porte du nul, pliage, veto de répétition, retrait du
// carré, tous les Fisher, tous les pourcentages) a donc tourné sur un
// mélange de matchs réels et de matchs FIFA, sans distinction. Aucune
// de ces mesures ne peut être considérée comme valable telle quelle.
//
// CE QUI EST SUPPRIMÉ : les 90 entrées de CAS_REFERENCE_V7 (Juventus,
// Inter, Roma, tout l'historique jusqu'au lot du 05/09 inclus), sans
// trace, comme demandé.
//
// CE QUI RESTE, ET POURQUOI : les RÈGLES elles-mêmes — la porte 6/12,
// le pliage M2⊕M7, le veto de répétition, les lois exactes du carré
// (M15 = somme des déplacements miroir, etc.) — ne sont pas des
// données, ce sont des structures ou des mécanismes. Elles restent
// dans le code, actives ou non selon BRANCHES_V7. Mais tout chiffre
// qui les accompagne dans les commentaires (« p = 0,0025 sur 15 cas »,
// « camp 48/63 ») décrit une mesure faite AVANT la suppression, sur
...
// ☠️ 14/09/26 : le test qui protégeait cette entrée dans l'archive n'a
// plus rien à protéger — CAS_REFERENCE_V7 a été vidée sur instruction
// d'Ellemine_D (mélange matchs réels/FIFA jamais filtré, cf.
// CAS_REFERENCE_V7_SUPPRIMEE_LE). LOT_0509_RESULTAT_V7 ci-dessus reste
// comme bilan écrit de ce qui s'était passé — il ne peut plus être
// recalculé depuis l'archive, seulement lu comme un constat daté.
