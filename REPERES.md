...
| **Ancrage direct** | Compare la force de l'ancre directe (binôme du chef) entre M1 et M7 — le camp dont l'ancre est la plus forte l'emporte. | **PRIORITAIRE sur l'écart de dominance depuis le 17/07/26** (demande explicite utilisateur) — validé seul 16/27, plus faible que l'écart de dominance qu'il devance désormais dans la cascade (le score mesuré de `verdictFinal` en a payé le prix, 19/27→17/27, assumé). |
...
| 2c | **Ancrage direct** | force de `BINOMES_V7[chef]` comparée M1/M7, dans `verdictFinal` | **16/27 (59%) seule** | **Actif, PRIORITAIRE sur l'écart de dominance** (demande explicite utilisateur, 17/07/26) — fait baisser le score mesuré de `verdictFinal` de 19/27 à **17/27** (assumé, documenté dans le commit) |
| 3 | Plus grand écart de dominance | bloc `carteRot`/`carteFixe` dans `verdictFinal` | 20/27 (74%) sur l'archive, seul | N'est plus le premier mécanisme consulté depuis l'ajout de l'ancrage direct (2c) — ne s'exprime que si l'ancrage ne tranche pas |
