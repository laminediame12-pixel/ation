...
| Match FIFA (19/07/26) M1=Caput Draconis/M7=Cauda Draconis | caput_draconis/puella/conjunctio/carcer | ? | — | `verdictFinal` prédit **Équipe 1** via ⚔️ GUERRE CIVILE (M7 trahi par son binôme : Acquisitio en M5 frappe Laetitia et protège ainsi Via, l'arme adverse). **Calculé directement avec `competition='esport_fc26_rush'`** (leçon du correctif ci-dessus) et vainqueur aligné sur `verdictFinal` : **score 5-2 Équipe 1**. `matchFermeOuvert` signale FERMÉ (8/16) mais, en esport, ça ne présage PAS un score bas (déjà 7 buts prédits ici). M1=Caput Draconis est une figure Air (signal de fragilité, voir §6) — à surveiller si Équipe 1 perd malgré la prédiction. Résultat réel non encore connu |
